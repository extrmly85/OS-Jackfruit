...
struct monitor_entry {
    ...
    int               soft_warned;   /* 1 once soft warning has been logged */
};

/* ------------------------------------------------------------------ */
/* TODO 2: Global list and lock                                       */
/*                                                                    */
/* We use a mutex (not a spinlock) because the ioctl path calls       */
/* kmalloc(..., GFP_KERNEL) which can sleep; a spinlock would be      */
/* illegal there.  The timer callback is fine with a mutex because it */
/* runs in a softirq context in which sleeping is not allowed — so we */
/* use mutex_trylock() in the timer path to avoid blocking.           */
/* ------------------------------------------------------------------ */
...
/* ------------------------------------------------------------------ */
/* Provided: soft / hard limit helpers                                */
/* ------------------------------------------------------------------ */
...
/* ------------------------------------------------------------------ */
/* TODO 3: Timer callback - periodic memory check                     */
/* ------------------------------------------------------------------ */
...
static void timer_callback(struct timer_list *t)
{
    ...
    /*
     * Use mutex_trylock so we never block in the softirq context.
     * If we cannot get the lock this tick, we simply skip; the next
     * tick (1 second later) will retry.
     */
...
static long monitor_ioctl(struct file *f, unsigned int cmd, unsigned long arg)
{
    ...
    /* Null-terminate the container_id for safety */
    ...
    /* ----------------------------------------------------------------
     * TODO 5: UNREGISTER - remove matching entry
     * ---------------------------------------------------------------- */
    printk(KERN_INFO
...
/* ------------------------------------------------------------------ */
/* Provided + TODO 6: Module Exit - free all remaining entries        */
/* ------------------------------------------------------------------ */
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Sudhanwa - PES2UG24CS531");
